...
create table if not exists public.ep_students (
 ...
 full_name text not null, email text, phone text, company text, level text,
...
-- Student-tier roster sync. Keep this in sync with migration 202607280002.
alter table public.ep_students add column if not exists user_id uuid references auth.users(id) on delete set null;
create unique index if not exists ep_students_user_id_unique on public.ep_students(user_id) where user_id is not null;
create or replace function public.ep_sync_student_from_profile() returns trigger language plpgsql security definer set search_path=public as $$ begin
 if lower(coalesce(new.tier,''))='student' then
  insert into public.ep_students(user_id,full_name,email,status,start_date)
  values(new.id,coalesce(nullif(trim(new.full_name),''),split_part(new.email,'@',1),'English Peak Student'),new.email,'Active',current_date)
  on conflict(user_id) where user_id is not null do update set full_name=excluded.full_name,email=excluded.email,status=case when ep_students.status='Prospect' then 'Active' else ep_students.status end;
 end if; return new; end $$;
drop trigger if exists ep_sync_student_from_profile on public.profiles;
create trigger ep_sync_student_from_profile after insert or update of tier,full_name,email on public.profiles for each row execute function public.ep_sync_student_from_profile();
insert into public.ep_students(user_id,full_name,email,status,start_date)
