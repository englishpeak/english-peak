...
test("Set 9 has the exact mixed CEFR content and supports every exercise mode", () => {
  ...
  const expectedPrompts = [
    ...
    "Últimamente he estado intentando cocinar más en casa.",
    "Si vuelves a encontrarte con ese problema, avísame de inmediato.",
    "No encuentro un enchufe cerca de la cama para cargar el teléfono.",
    "No fue hasta que revisé mi estado de cuenta que me di cuenta de que me habían cobrado dos veces.",
    "Por más razonable que parezca la explicación, todavía no aborda la causa principal del problema.",
    "Mi papá toma café todas las mañanas.",
    "¿Ya decidiste qué vas a llevar al viaje?",
    "¿Podrías bajar las persianas? El sol me está dando directamente en la cara.",
    "No me esperaba que la conversación tomara ese rumbo.",
    "Si hubiéramos salido a tiempo, no estaríamos atrapados en el tráfico ahora.",
    "Esta mochila es demasiado pesada.",
    "Llevo meses intentando convencerlo de que cambie de opinión.",
    "¿Sabes qué trámite tengo que hacer para renovar el permiso?",
    "No podemos seguir dependiendo de una solución temporal.",
    "Por lo que he podido averiguar, el problema afecta principalmente a los usuarios más antiguos.",
    "¿Hay un cajero automático cerca de aquí?",
    "No tenía intención de sonar grosero; simplemente estaba cansado.",
    "La puerta hace ruido porque una de las bisagras está floja.",
    "Nunca había visto tanta gente en un solo lugar.",
    "No descartaría la posibilidad de que hayan interpretado mal nuestras intenciones.",
    "¿Puedes guardar esto en el refrigerador?",
    "Pensé que ya habría terminado para cuando llegaras.",
    "Tuvieron que cerrar parte de la banqueta mientras reparaban la tubería.",
    "Me cuesta creer que nadie se haya dado cuenta antes.",
    "Lejos de resolver el problema, la medida parece haber agravado una situación que ya era bastante delicada.",
    "¿Por qué no tomamos un taxi en lugar de esperar el autobús?",
    "Me habría ahorrado mucho tiempo si alguien me hubiera explicado el proceso desde el principio.",
    "El banco rechazó mi solicitud de contracargo porque había pasado demasiado tiempo desde la compra.",
    "Por muy tentador que resulte atribuir el fracaso a una sola decisión, hacerlo sería pasar por alto una serie de factores que venían acumulándose desde hacía años."
  ];
  const expectedLevels = ["A1","B1","A2","C1","B2","C1","A1","A2","B1","B1","B2","A1","B1","B2","B2","C1","A1","B1","B1","A2","C1","A1","B2","A2","C1","C2","A2","B2","C1","C2"];
  ...
  assert.ok(set9);
  assert.equal(set9.title, "Set 9");
  assert.equal(set9.description, "30 sentences · Mixed levels · Mixed grammar and vocabulary");
  assert.deepEqual(Array.from(set9.sentences, sentence => sentence.spanish), expectedPrompts);
  assert.deepEqual(Array.from(set9.sentences, sentence => sentence.level), expectedLevels);
  assert.deepEqual(Array.from(set9.sentences, sentence => sentence.id), Array.from({ length: 30 }, (_, index) => index + 1));
  assert.deepEqual(Object.fromEntries(["A1","A2","B1","B2","C1","C2"].map(level => [level, set9.sentences.filter(sentence => sentence.level === level).length])), { A1:5, A2:5, B1:6, B2:6, C1:6, C2:2 });

  set9.sentences.forEach(sentence => {
  ...
  const vocabularyChallenges = {
    4: ["enchufe", "outlet"],
    9: ["persianas", "blinds"],
    14: ["trámite", "paperwork"],
    19: ["bisagras", "hinges"],
    24: ["banqueta", "sidewalk"],
    29: ["contracargo", "chargeback"]
  };
  assert.equal(Object.keys(vocabularyChallenges).length, 6);
  assert.equal(Object.keys(vocabularyChallenges).length / set9.sentences.length, 0.2);
  for (const [id, [spanishTarget, englishTarget]] of Object.entries(vocabularyChallenges)) {
    const sentence = set9.sentences[Number(id) - 1];
  ...
  assert.doesNotMatch(set9.sentences[3].primaryAnswer.toLowerCase(), /plug/);
  assert.doesNotMatch(set9.sentences[8].acceptedAnswers.join(" ").toLowerCase(), /curtain/);
  assert.match(set9.sentences[13].note, /no single fixed equivalent/i);
  assert.doesNotMatch(set9.sentences[23].primaryAnswer.toLowerCase(), /bench/);
  assert.doesNotMatch(set9.sentences[28].acceptedAnswers.join(" ").toLowerCase(), /refund/);
  assert.match(set9.sentences[12].acceptedAnswers.join(" "), /convince her/);
});

test("Set 8 has the exact mixed CEFR content and supports every exercise mode", () => {
  const sets = evaluateSets(html.slice(dataStart, expansionStart));
  const set8 = sets.find(set => set.id === 8);
  const expectedPrompts = [
    "¿Dónde dejaste tu teléfono?",
    "Llevo unos días pensando en cambiar de trabajo.",
    "Si me hubieras avisado antes, podría haberte conseguido una entrada.",
    "La tienda cierra a las nueve.",
    "Por lo que entendí, todavía no han descartado ninguna de las opciones.",
    "Me dio pena interrumpirlos, así que esperé afuera.",
    "¿Con qué frecuencia haces ejercicio?",
    "No deberíamos sacar conclusiones hasta tener toda la información.",
    "Resulta que el vuelo había sido cancelado horas antes de que llegáramos al aeropuerto.",
    "¿Me puedes prestar un bolígrafo?",
    "No estoy muy seguro de cómo abordar este problema.",
    "Llevo media hora tratando de comunicarme con atención al cliente.",
    "No importa cuánto insistas, no voy a cambiar de opinión.",
    "Hay demasiada gente aquí.",
    "La nueva política entrará en vigor a principios del próximo mes.",
    "¿Qué te impide pedirle ayuda?",
    "Si no tienes prisa, podemos ir caminando.",
    "A estas alturas, deberíamos haber recibido alguna respuesta.",
    "Me sorprendió lo rápido que logró adaptarse al nuevo puesto.",
    "Necesito comprar algo para cenar.",
    "No quiero adelantarme a los acontecimientos, pero todo parece indicar que aceptarán la propuesta.",
    "Se me pasó por completo que hoy era su cumpleaños.",
    "¿Qué habrías hecho en mi lugar?",
    "El proyecto se retrasó porque varios proveedores no cumplieron con los plazos acordados.",
    "¿Puedes hablar un poco más despacio?",
    "Por mucho que intentemos racionalizar la decisión, es difícil ignorar las consecuencias a largo plazo.",
    "Terminé deshaciéndome de varias cosas que ya no usaba.",
    "Me parece que estamos perdiendo de vista el problema principal.",
    "Una vez que hayas enviado la solicitud, ya no podrás modificarla.",
    "Difícilmente podría haberse previsto que una medida aparentemente insignificante desencadenaría una reacción de semejante magnitud."
  ];
  const expectedLevels = ["A1","B1","B2","A1","C1","B1","A2","C1","B2","A1","C1","B1","B2","A1","C1","B1","A2","B2","B1","A1","C1","A2","B2","B2","A2","C2","B1","C1","A2","C2"];
  const normalize = value => String(value).toLowerCase().replace(/[’‘`´]/g, "'").replace(/[¿?¡!]/g, " ").replace(/[.,;]/g, " ").replace(/\s+/g, " ").trim();
  const easyWords = value => value.replace(/[’‘`´]/g, "'").replace(/[.,;:!?¿¡]/g, "").split(/\s+/).filter(Boolean);

  assert.ok(set8);
  assert.equal(set8.title, "Set 8");
  assert.equal(set8.description, "30 sentences · Mixed levels · Mixed grammar and vocabulary");
  assert.deepEqual(Array.from(set8.sentences, sentence => sentence.spanish), expectedPrompts);
  assert.deepEqual(Array.from(set8.sentences, sentence => sentence.level), expectedLevels);
  assert.deepEqual(Array.from(set8.sentences, sentence => sentence.id), Array.from({ length: 30 }, (_, index) => index + 1));
  assert.deepEqual(Object.fromEntries(["A1","A2","B1","B2","C1","C2"].map(level => [level, set8.sentences.filter(sentence => sentence.level === level).length])), { A1:5, A2:5, B1:6, B2:6, C1:6, C2:2 });

  set8.sentences.forEach(sentence => {
    assert.deepEqual(Object.keys(sentence).sort(), ["acceptedAnswers", "id", "level", "mediumPrompt", "note", "primaryAnswer", "spanish"]);
    assert.ok(sentence.acceptedAnswers.length >= 3 && sentence.acceptedAnswers.length <= 6);
    assert.ok(sentence.mediumPrompt.split(/\s+/).length >= 2 && sentence.mediumPrompt.split(/\s+/).length <= 3);
    assert.ok(sentence.primaryAnswer.startsWith(sentence.mediumPrompt));
  ...
  assert.ok(set8.sentences[15].acceptedAnswers.some(answer => answer.includes("her for help")));
  assert.ok(set8.sentences[18].acceptedAnswers.some(answer => answer.includes("he managed")));
  assert.ok(set8.sentences[21].acceptedAnswers.some(answer => answer.includes("her birthday")));
});

test("Set 8 adds plenty of curated alternatives, with extra range at higher levels", () => {
  const originalSets = evaluateSets(html.slice(dataStart, expansionStart));
  const expandedSets = evaluateSets(html.slice(dataStart, appStart));
  const originalSet8 = originalSets.find(set => set.id === 8);
  const set8 = expandedSets.find(set => set.id === 8);
  const minimumByLevel = { A1: 4, A2: 4, B1: 6, B2: 6, C1: 8, C2: 8 };

  set8.sentences.forEach((sentence, index) => {
    const originalAnswers = new Set(originalSet8.sentences[index].acceptedAnswers.map(answer => answer.toLowerCase()));
    const additions = new Set(sentence.acceptedAnswers.map(answer => answer.toLowerCase()).filter(answer => !originalAnswers.has(answer)));
    assert.ok(
      additions.size >= minimumByLevel[sentence.level],
      `Set 8 sentence ${sentence.id} (${sentence.level}) should gain at least ${minimumByLevel[sentence.level]} accepted alternatives`
    );
  });

  assert.ok(set8.sentences[2].acceptedAnswers.includes("Had you let me know earlier, I might have been able to get you a ticket."));
  assert.ok(set8.sentences[7].acceptedAnswers.includes("We should reserve judgment until we have the complete information."));
  assert.ok(set8.sentences[20].acceptedAnswers.includes("I don't want to speak too soon, but everything suggests that they will accept the proposal."));
  assert.ok(set8.sentences[25].acceptedAnswers.includes("Try as we might to justify the decision, we cannot overlook its long-term consequences."));
...
test("Set 7 adds three to six curated alternatives for every prompt", () => {
  const originalSets = evaluateSets(html.slice(dataStart, expansionStart));
  ...
  set7.sentences.forEach((sentence, index) => {
    ...
    assert.ok(addedCount >= 3 && addedCount <= 6, `Sentence ${index + 1} should gain three to six accepted alternatives`);
  });
  ...
  assert.ok(set7.sentences[25].acceptedAnswers.includes("Could you come and get me at the airport?"));
