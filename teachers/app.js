...
function studentDetail(id){const s=state.students.find(x=>x.id===id);if(!s)return title('Student unavailable','The record does not exist or you are not authorized to access it.');const a=state.assignments.find(x=>x.student_id===id&&x.status==='Active');return title(esc(s.full_name),`${esc(s.level)} · ${esc(s.status)}`,`<div class="quick"><button class="btn" data-edit-student="${s.id}">Edit Student</button>${state.isAdmin?'<button class="btn" data-action="assign">Reassign</button>':''}</div>`)+`<div class="tabs"><span>Overview</span><span>Schedule & Classes</span><span>Tasks</span><span>Notes</span><span>History</span></div><div class="grid"><article class="ep-card card"><h3>Basic information</h3><p>${esc(s.email)}<br>${esc(s.phone)}<br>${esc(s.company)}<br>${esc(s.timezone)}${s.start_date?`<br>Started ${esc(s.start_date)}`:''}</p></article><article class="ep-card card"><h3>Current teacher</h3><p>${esc(teacherName(a?.teacher_user_id))}</p><p>${esc(s.classes_per_week)} classes/week</p></article></div>`+table(['Date','Time','Status','Notes'],state.sessions.filter(x=>x.student_id===id).map(x=>`<tr><td>${x.class_date}</td><td>${x.start_time}</td><td>${badge(x.status)}</td><td>${esc(x.notes)}</td></tr>`).join(''))}
...
function stepperField(label,name,value,min=0,required=''){return `<div class="field"><label>${label}</label><div class="number-stepper"><div class="stepper-buttons"><button type="button" data-step="1" aria-label="Increase ${label}">▲</button><button type="button" data-step="-1" aria-label="Decrease ${label}">▼</button></div><input name="${name}" type="number" value="${esc(value)}" min="${min}" ${required}></div></div>`}
function optionalStartDate(value=''){const enabled=Boolean(value);return `<div class="optional-field"><div class="optional-field-head"><span>Start date</span><label class="switch"><input type="checkbox" data-start-date-toggle ${enabled?'checked':''}><span class="switch-track" aria-hidden="true"></span><span data-toggle-label>${enabled?'On':'Off'}</span></label></div><div class="field start-date-field ${enabled?'':'hidden'}"><input name="start_date" type="date" value="${esc(value)}" ${enabled?'':'disabled'}></div></div>`}
function studentForm(){openModal(`<h2 class="ep-heading">Add Student</h2><form id="studentForm">${formField('Full name','full_name','text','required')}${formField('Email','email','email')}${formField('Level','level')}${optionalStartDate()}<div class="field"><label>Status</label><select name="status">${['Active','Paused','Inactive','Prospect'].map(x=>`<option>${x}</option>`)}</select></div>${stepperField('Classes per week','classes_per_week','',0)}<button class="ep-btn-primary">Save Student</button></form>`)}
function studentEditForm(id){const s=state.students.find(x=>x.id===id);if(!s)return;openModal(`<h2 class="ep-heading">Edit Student</h2><p class="form-note">Update academic and contact details. Account matching remains an Admin-only action.</p><form id="studentEditForm" data-id="${id}">${formField('Full name','full_name','text',`required value="${esc(s.full_name)}"`)}${formField('Email','email','email',`value="${esc(s.email||'')}"`)}${formField('Phone','phone','tel',`value="${esc(s.phone||'')}"`)}${formField('Company','company','text',`value="${esc(s.company||'')}"`)}${formField('Level','level','text',`value="${esc(s.level||'')}"`)}${optionalStartDate(s.start_date)}<div class="field"><label>Status</label><select name="status">${['Active','Paused','Inactive','Prospect'].map(x=>`<option ${x===s.status?'selected':''}>${x}</option>`)}</select></div>${stepperField('Classes per week','classes_per_week',s.classes_per_week??'',0)}${formField('Timezone','timezone','text',`required value="${esc(s.timezone||'America/Mexico_City')}"`)}<div class="field"><label>Academic notes</label><textarea name="academic_notes">${esc(s.academic_notes||'')}</textarea></div><button class="ep-btn-primary">Save Changes</button></form>`)}
