...
function classes(){
  ...
  return title(state.isAdmin?'Classes':'My Classes','Students may join multiple classes, and every class can have a teaching team.',state.isAdmin?'<button class="ep-btn-primary" data-action="class">Add Class</button>':'')+
    table(['Class','Type','Weekly','Students','Main teacher','Secondary teachers','Status'],visible.map(item=>{
      ...
      return `<tr><td><strong>${esc(item.name)}</strong></td><td><div class="type-list">${classTypes(item.class_type).map(type=>badge(type)).join('')}</div></td><td>${item.classes_per_week}</td><td>${esc(students.join(', ')||'No students')}</td><td>${esc(teacherName(main?.teacher_user_id))}</td><td>${esc(secondary.join(', ')||'—')}</td><td>${badge(item.status)}</td></tr>`;
