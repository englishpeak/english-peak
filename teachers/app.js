...
const state = { user: null, profile: null, isAdmin: false, teachers: [], studentAccounts: [], students: [], assignments: [], sessions: [], reports: [], tasks: [] };
...
async function loadData() {
  ...
  if (state.isAdmin) {
    [state.teachers,state.studentAccounts] = await Promise.all([
      query('profiles','id,email,full_name,tier,is_admin',q=>q.eq('tier','teacher')),
...
function directory(){return title('Teachers','Existing accounts with the Teacher tier.')+table(['Teacher','Email','Status','Students','Hours this week','Report','Pending tasks',''],state.teachers.map(t=>{
  ...
  return `<tr><td>${esc(t.full_name||'Teacher')}</td><td>${esc(t.email)}</td><td>${badge('Active')}</td><td>${count}</td><td>${(minutes/60).toFixed(1)}</td><td>${badge(report?.status||'Missing')}</td><td>${state.tasks.filter(x=>x.assigned_teacher_user_id===t.id&&x.status!=='Completed').length}</td><td><a href="/teachers/directory/${t.id}">Open</a></td></tr>`}).join(''))}
...
function students(){return title(state.isAdmin?'Students':'My Students','Student records and active teacher assignments.',state.isAdmin?'<button class="ep-btn-primary" data-action="student">Add Student</button>':'')+
  ...
  table(['Student','Email','Account','Level','Status','Teacher',''],state.students.map(s=>{const a=state.assignments.find(a=>a.student_id===s.id&&a.status==='Active');return `<tr data-student-row data-search="${esc((s.full_name+' '+(s.email||'')).toLowerCase())}" data-status="${s.status}"><td>${esc(s.full_name)}</td><td>${esc(s.email)}</td><td>${s.account_matched_at?badge('Matched'):s.user_id?badge('Registered'):`<button class="btn compact" data-match="${s.id}">Match account</button>`}</td><td>${esc(s.level)}</td><td>${badge(s.status)}</td><td>${esc(teacherName(a?.teacher_user_id))}</td><td><a href="/teachers/students/${s.id}">Open</a></td></tr>`}).join(''),'No students assigned yet.')}
...
document.addEventListener('submit',async e=>{e.preventDefault();const f=e.target,d=Object.fromEntries(new FormData(f));try{if(f.id==='matchForm')await confirmStudentMatch(f,d);if(f.id==='studentForm')await mutate(sb.from(ACADEMIC_TABLES.students).insert({...d,classes_per_week:Number(d.classes_per_week)||null,default_class_duration:Number(d.default_class_duration)}),'Student created.');if(f.id==='assignForm'){await sb.from(ACADEMIC_TABLES.assignments).update({status:'Ended',end_date:d.start_date}).eq('student_id',d.student_id).eq('status','Active');await mutate(sb.from(ACADEMIC_TABLES.assignments).insert({...d,status:'Active'}),'Student assigned.')}if(f.id==='taskForm')await mutate(sb.from(ACADEMIC_TABLES.tasks).insert({...d,assigned_by_user_id:state.user.id,status:'Not Started'}),'Task created.');if(f.id==='commentForm')await mutate(sb.from(ACADEMIC_TABLES.comments).insert({task_id:f.dataset.id,author_user_id:state.user.id,comment:d.comment}),'Comment preserved.');if(f.id==='reportForm'){if(!confirm('Submit this weekly report? You will not be able to edit it unless an admin requests changes.'))return;const week=getWeekRange(),sessions=state.sessions.filter(s=>s.teacher_user_id===state.user.id),t=calculateReport(sessions);await mutate(sb.from(ACADEMIC_TABLES.reports).upsert({teacher_user_id:state.user.id,week_start:week.start,week_end:week.end,total_completed_minutes:t.totalCompletedMinutes,total_billable_minutes:t.totalBillableMinutes,total_completed_classes:t.totalCompletedClasses,total_no_shows:t.totalNoShows,status:'Submitted',teacher_comments:d.teacher_comments,submitted_at:new Date().toISOString()},{onConflict:'teacher_user_id,week_start'}),'Weekly report submitted.')}}catch(err){alert(err.message)}});
