...
const state = { user: null, profile: null, isAdmin: false, teachers: [], studentAccounts: [], students: [], assignments: [], classes: [], classStudents: [], classTeachers: [], sessions: [], reports: [], tasks: [] };
...
function classes(){
  ...
  return title(state.isAdmin?'Classes':'My Classes','Students may join multiple classes, and every class can have a teaching team.',state.isAdmin?'<button class="ep-btn-primary" data-action="class">Add Class</button>':'')+
    table(['Class','Type','Weekly','Students','Main teacher','Secondary teachers','Status'],visible.map(item=>{
      ...
      return `<tr><td><strong>${esc(item.name)}</strong></td><td>${esc(item.class_type)}</td><td>${item.classes_per_week}</td><td>${esc(students.join(', ')||'No students')}</td><td>${esc(teacherName(main?.teacher_user_id))}</td><td>${esc(secondary.join(', ')||'—')}</td><td>${badge(item.status)}</td></tr>`;
...
async function createClass(form,data){
  ...
  const result=await sb.from(ACADEMIC_TABLES.classes).insert({name:data.name,class_type:data.class_type,classes_per_week:Number(data.classes_per_week),status:'Active',created_by_user_id:state.user.id}).select('id').single();
