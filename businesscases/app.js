...
const caseImage = (item, className = '', loading = 'lazy') => `<div class="image-frame ${className}"><img src="${esc(item.imageUrl)}" alt="${esc(item.imageAlt || '')}" loading="${esc(loading)}" decoding="async"><div class="image-placeholder" hidden aria-hidden="true"><span>BC</span><small>Image unavailable</small></div></div>`;
...
function audioMarkup(c) {
  ...
  return `${context}<div class="audio-player"><audio preload="metadata" src="${esc(c.listening.audioUrl)}"></audio><div class="audio-controls"><button class="icon-btn play" aria-label="Play audio">▶</button><button class="icon-btn rewind" aria-label="Rewind 10 seconds">↶ 10</button><button class="icon-btn forward" aria-label="Fast forward 10 seconds">10 ↷</button><span class="time">0:00 / 0:00</span><label>Volume <input class="volume" type="range" min="0" max="1" step=".05" value="1"></label><label>Speed <select class="speed"><option>.75</option><option selected>1</option><option>1.25</option><option>1.5</option></select>×</label></div><input class="progress" aria-label="Audio progress" type="range" min="0" max="100" value="0"><p class="audio-error" hidden>The audio could not be loaded. Please check your connection and try again.</p></div><button class="btn script-toggle" type="button" aria-expanded="false">Show Script</button><div class="script-wrap closed"><div class="script-panel"><h3>${esc(c.listening.title)}</h3>${c.listening.transcript.map(transcriptLineMarkup).join('')}</div></div>`;
}
